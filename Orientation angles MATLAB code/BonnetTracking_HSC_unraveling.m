...
video = "new_7cm_6Hz";
name = "new_7cm_6Hz";
videoname = "fresh_sample\260115_new_sample\" + video + "\" + video + ".avi";
...
storeaspect = zeros(frames,1);
storearea = zeros(frames,1);
storeecc = zeros(frames,1);
%storedangle(1) = or.Orientation;

%Create Videowriter object
video_name = name + "_direction.avi";
...
while hasFrame(vid)   
    ...
    or = regionprops(bin2,'centroid','Orientation', "MajorAxisLength","MinorAxisLength", "Area", "Eccentricity");
    
    angle = or.Orientation;
    storeaspect(i) = or.MajorAxisLength/or.MinorAxisLength;
    storearea(i) = or.Area;
    storeecc(i) = or.Eccentricity;
    % unravel data to 2pi
...
if i >= 2
    prev = storedangle(i-1);
    angle = angle - 180*round((angle - prev)/180);
end
...
%plot(x,y - 1.333800025967362,".",'MarkerSize',10)
%set(gcf,'color','w')
%set(gca,'fontsize',16)
%ylabel('Bonnet Angle (°)')
%xlabel('Time (s)') 
...
figure;
plot(x, storedangle, '.', 'MarkerSize', 10)
ylabel('Angle (deg)')
xlabel('Time (s)')

% figure;
% plot(x, storeaspect,'.', 'MarkerSize', 10)
% ylabel('aspect ratio')
% xlabel('Time (s)')
% 
% figure;
% plot(x, storearea,'.', 'MarkerSize', 10)
% ylabel('area')
% xlabel('Time (s)')
% 
% figure;
% plot(x, storeecc,'.', 'MarkerSize', 10)
% ylabel('eccentricity')
% xlabel('Time (s)')


ExcelMatrix = table(x, storedangle, 'VariableNames',{'Time', 'Angle'});
excelfile = "Angles" + name + ".csv";
writetable(ExcelMatrix, excelfile)